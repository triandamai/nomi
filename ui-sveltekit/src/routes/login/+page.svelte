<script lang="ts">
    ...
    async function handleVerifyOtp(e: Event) {
        ...
        try {
            ...
            if(response.meta.code >= 200 && response.meta.code <= 209) {
                setSession(response.data.access_token, response.data.user_id)
                await goto('/chat');
            ...
            <div class="relative space-y-8" in:fly={{ y: 20, duration: 800 }}>
                ...
                <div class="text-center space-y-4">
                    ...
                    <div class="flex justify-center gap-6">
                        ...
                        <a href="/privacy" class="text-xs text-slate-500 hover:text-white transition-colors">Security
                            Protocol</a>
                    </div>
                </div>
            </div>
