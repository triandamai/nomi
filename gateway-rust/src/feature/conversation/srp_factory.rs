...
use crate::feature::edge_functions::{CreateEdgeFunctionRequest, handle_create_edge_function, handle_update_edge_function};
...
pub async fn update_proposal(
    ...
    let intents: Option<Vec<String>> = payload["intents"]
        .as_array()
        .map(|arr| {
            arr.iter()
    ...
    match record {
        Ok(Some(row)) => {
            ...
            let deployment_res = if exists {
                handle_update_edge_function(State(state.clone()), Extension(claims), Path(slug.clone()), Json(payload)).await
            } else {
                handle_create_edge_function(State(state.clone()), Extension(claims), Json(payload)).await
            };

            if deployment_res.meta.code != 200 {
                return ApiResponse::create(deployment_res.meta.code, "".to_string(), &deployment_res.meta.message);
            }
            ...
            ApiResponse::ok("".to_string(), "Plugin hot-patched into production successfully via standard deployment pipeline.")
        },
